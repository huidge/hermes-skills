...
### WeChat/Weixin QR code not appearing (macOS SSL error)

**Symptom:** Logs show `SSLCertVerificationError: certificate verify failed: unable to get local issuer certificate` when connecting to `ilinkai.weixin.qq.com`.

**Cause:** macOS Python 3.x installation missing SSL root certificates. Python's ssl module can't find the system CA bundle, so all HTTPS connections fail certificate verification. `curl` works because it uses the system certificate store directly.

**Fix — run the Install Certificates script:**
```bash
"/Applications/Python 3.11/Install Certificates.command"
```
This installs certifi and symlinks the CA bundle into Python's SSL path. Then restart the gateway:
```bash
...
**Verify fix:**
```bash
python3 -c "import urllib.request; urllib.request.urlopen('https://ilinkai.weixin.qq.com/ilink/bot/get_bot_qrcode?bot_type=3', timeout=10); print('SSL OK')"
```

**If the script doesn't exist:** Install certifi manually and set the env var:
```bash
pip install certifi
export SSL_CERT_FILE=$(python3 -c "import certifi; print(certifi.where())")
```
Add the `export` line to `~/.zshrc` to persist across sessions.
